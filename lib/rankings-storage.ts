...
// Local override — generated by scripts/build-local-rankings.mjs.
// When present, wins over both Redis and SEED_RANKINGS so the Rankings +
// Pools tabs work without needing prod Redis. Falls back to a null export
// (which we treat as "no override") if the file hasn't been generated.
// The `unknown` cast + type-narrowing keeps this safe when the import
// resolves to either a bundle or null.
import { LOCAL_RANKINGS as _LOCAL } from "./local-rankings";
const LOCAL_OVERRIDE: RankingsBundle | null =
  (_LOCAL as unknown as RankingsBundle | null) ?? null;
...
export async function getRankings(): Promise<RankingsBundle | null> {
  // Local override wins. Bypass Redis entirely so we don't inherit stale
  // state from an accidentally shared workspace.
  if (LOCAL_OVERRIDE) return withTimestamp(LOCAL_OVERRIDE);
